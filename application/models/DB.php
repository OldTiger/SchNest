<?php
define("_DB_SER", "localhost");
define("_DB_USER", "root");
define("_DB_PASS", "admin");
define("_DB_NAME", "schnest");
define("_DB_LOG", true);
define("_DB_LOG_FILE","/");
class DB {
		private $link_id;
		...
		public function __construct() {
			...
			$this->connect(_DB_SER, _DB_USER, _DB_PASS, _DB_NAME);
			$this->is_log = _DB_LOG;
			if($this->is_log){
				$handle = fopen(_DB_LOG_FILE."dblog.txt", "a+");
				$this->handle=$handle;
			}
		}
		...
		public function update( $table,$dataArray,$condition="") {
			if( !is_array($dataArray) || count($dataArray)<=0) {
				$this->halt('there is no data for update');
				return false;
			}
			$value = "";
			while( list($key,$val) = each($dataArray))
